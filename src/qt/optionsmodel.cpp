...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeeonagenAmount"))
        settings.setValue("nAnonymizeeonagenAmount", 1000);

    nDarksendRounds = settings.value("nDarksendRounds").toLongLong();
    nAnonymizeeonagenAmount = settings.value("nAnonymizeeonagenAmount").toLongLong();
    ...
    if (settings.contains("nAnonymizeeonagenAmount"))
        SoftSetArg("-anonymizeeonagenamount", settings.value("nAnonymizeeonagenAmount").toString().toStdString());
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case anonymizeeonagenAmount:
            nAnonymizeeonagenAmount = value.toInt();
            settings.setValue("nAnonymizeeonagenAmount", nAnonymizeeonagenAmount);
            emit anonymizeeonagenAmountChanged(nAnonymizeeonagenAmount);
            break;
#ifdef USE_NATIVE_I2P
        case I2PUseI2POnly:
        {
