...
{
    // Convert eonagen:// to eonagen:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("eonagen://"))
    {
        uri.replace(0, 12, "eonagen:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "eonagen.lnk";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart)
    {
        ...
        // Get a pointer to the IeonagenLink interface.
        IShellLink* psl = NULL;
        ...
        if (SUCCEEDED(hres))
        {
            ...
            // Query IeonagenLink for the IPersistFile interface for
...
{
    return GetAutostartDir() / "eonagen.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("eonagen-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  eonagen-qt [" + tr("command-line options") + "]                     " + "\n";
