...
const string strMessageMagic = "eonagen Signed Message:\n";
                        ...
                        bool* pfMissingInputs)
{
    ...
    {
    LOCK(pool.cs); // protect pool.maEONAxtTx
    for (unsigned int i = 0; i < tx.vin.size(); i++)
    {
        COutPoint outpoint = tx.vin[i].prevout;
        if (pool.maEONAxtTx.count(outpoint))
        {
...
{
    {
        ...
        BOOST_FOREACH(PAIRTYPE(const uint256, CBlockIndex*)& item, mapBlockIndex)
        {
            ...
            if (pindex == pindexBest || pindex->EONAxt != 0)
                continue;
            CBlock block;
...
CBlockIndex* FindBlockByHeight(int nHeight)
{
    ...
    while (pblockindex->nHeight < nHeight)
        pblockindex = pblockindex->EONAxt;
...
{
    int64_t nSubsidy = nCoinAge * COIN_YEAR_REWARD * 33 / (365 * 33 + 8);  // 80% yearly interest 
...
bool static Reorganize(CTxDB& txdb, CBlockIndex* pindexNew)
{
    ...
    BOOST_FOREACH(CBlockIndex* pindex, vDisconnect)
        if (pindex->pprev)
            pindex->pprev->EONAxt = NULL;

    // Connect longer branch
    BOOST_FOREACH(CBlockIndex* pindex, vConnect)
        if (pindex->pprev)
            pindex->pprev->EONAxt = pindex;
...
bool CBlock::SetBestChainInner(CTxDB& txdb, CBlockIndex *pindexNew)
{
    ...
    pindexNew->pprev->EONAxt = pindexNew;
...
void PrintBlockTree()
{
    ...
    map<CBlockIndex*, vector<CBlockIndex*> > maEONAxt;
    for (map<uint256, CBlockIndex*>::iterator mi = mapBlockIndex.begin(); mi != mapBlockIndex.end(); ++mi)
    {
        CBlockIndex* pindex = (*mi).second;
        maEONAxt[pindex->pprev].push_back(pindex);
        // test
        //while (rand() % 3 == 0)
        //    maEONAxt[pindex->pprev].push_back(pindex);
    }
    ...
    while (!vStack.empty())
    {
        ...
        vector<CBlockIndex*>& vNext = maEONAxt[pindex];
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    else if (strCommand == "getblocks")
    {
        ...
        for (; pindex; pindex = pindex->EONAxt)
        {
