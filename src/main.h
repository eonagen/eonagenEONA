...
#define START_MASTERNODE_PAYMENTS_TESTNET 1544992210 // Sunday, 16 October 2018 20:30:10
#define START_MASTERNODE_PAYMENTS 1545255922 //  Wednesday, 19 December 2018 21:45:22
...
#define INSTANTX_SIGNATURES_TOTAL              14
...
static const int64_t MAX_MONEY = 20000000000 * COIN; // 20 Billion
...
static const int64_t TARGET_SPACING = 45; // 45 second block times expected
 ...
 * candidates to be the next block.  pprev and EONAxt link a path through the
 * main/longest chain.  A blockindex may have multiple pprev pointing back
 * to it, but EONAxt will only point forward to the longest branch, or will
...
public:
    ...
    std::string ToString() const
    {
        return strprintf("CBlockIndex(nprev=%p, EONAxt=%p, nFile=%u, nBlockPos=%-6d nHeight=%d, nMint=%s, nMoneySupply=%s, nFlags=(%s)(%d)(%s), nStakeModifier=%016x, hashProof=%s, prevoutStake=(%s), nStakeTime=%d merkle=%s, hashBlock=%s)",
            pprev, EONAxt, nFile, nBlockPos, nHeight,
...
public:
    ...
    explicit CDiskBlockIndex(CBlockIndex* pindex) : CBlockIndex(*pindex)
    {
        ...
        hashNext = (EONAxt ? EONAxt->GetBlockHash() : 0);
    }
