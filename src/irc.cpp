...
void ThreadIRCSeed2()
{
	...
	int ntimeout = 1000;
        ...
        if (TestNet())
        {
            Send(hSocket, "JOIN #eonagencoinTEST\r");
            Send(hSocket, "WHO #eonagencoinTEST\r");
        } else
        {
	    LogPrintf("JOIN #eonagencoin\n");
            // randomly join #eonagencoin00-#eonagencoin05
            //int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #eonagencoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #eonagencoin%02d\r", channel_number).c_str());
        };
