...
#define DUMP_ADDRESSES_INTERVAL 305

using namespace std;
using namespace boost;

static const int MAX_OUTBOUND_CONNECTIONS = 76; // 76 outbound connections
...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "eonagen " + FormatFullVersion();
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. eonagen is probably already running."), addrBind.ToString());
        else
